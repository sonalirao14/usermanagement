...
//   deleteMany<T extends MongoDocument>(collectionName: string, query: Filter<T>): Promise<{ deletedCount: number }>;
// }

export interface IDatabase<T> {
  insertOne(collectionName: string, document: T): Promise<string>;
  findOne(collectionName: string, query: Record<string, any>): Promise<T | null>;
  findAll(collectionName: string, query?: Record<string, any>): Promise<T[]>;
  updateOne(collectionName: string, query: Record<string, any>, update: Partial<T>): Promise<boolean>;
  deleteOne(collectionName: string, query: Record<string, any>): Promise<boolean>;
  deleteMany(collectionName: string, query: Record<string, any>): Promise<{ deletedCount: number }>;
}